...
pub async fn log_layer(
    ...
    let url = format!("{} {}", req.method(), req.uri());
    ...
    tracing::warn!("{} {}ms {}", url, time, res.status());
    Ok(res)
}

pub async fn buffer_and_print<B>(
    body: B, is_request: bool, url: &String,
    ...
    B::Error: std::fmt::Display,
{
    ...
    if let Ok(body) = std::str::from_utf8(&bytes) {
        if is_request {
            ...
            let len = body.char_indices().nth(1024).unwrap_or((body.len(), ' '));
            let body = &body[..len.0];
