...
impl CasbinService {
    ...
    pub async fn get_users_for_role(role_id: impl ToString) -> Vec<String> {
        casbin_write!().get_users_for_role(&role_id.to_string(), None)
    }
    /// 获取用户的角色
    ...
    pub async fn delete_roles_policy(role_ids: &Vec<i32>) -> Result<bool> {
        for role_id in role_ids {
            let r = casbin_write!()
                .remove_filtered_policy(0, vec![format!("r{role_id}")])
                .await?;
            if !r {
                return Ok(false);
            }
    ...
    pub async fn update_role_policy(role_id: u64, menus: &[SysMenu]) -> Result<bool> {
        ...
        let current_policies = {
            let policies = casbin_write!().get_filtered_policy(0, vec![role_id.clone()]);
        ...
        for menu in menus
            .iter()
            .filter(|x| x.menu_type == 3 && !crate::is_empty!(x.api_url))
        {
        ...
        // 找出差异
        let policies_to_remove: Vec<_> = current_policies
            .difference(&new_policies)
            .cloned()
            .collect();

        let policies_to_add: Vec<_> = new_policies
    ...
    pub async fn update_user_roles(user_id: impl ToString, role_ids: &[u64]) -> Result<bool> {
        // 获取当前用户已有的角色
        let current_roles = casbin_write!().get_roles_for_user(&user_id.to_string(), None);

        // 将新的角色ID转换为预期的格式
        let new_roles: Vec<String> = role_ids.iter().map(|x| format!("r{x}")).collect();

        // 找出需要删除的角色
        let roles_to_remove: Vec<String> = current_roles
            .iter()
        ...
        if !roles_to_remove.is_empty() {
            for role in roles_to_remove {
                casbin_write!()
                    .remove_grouping_policy(vec_string![user_id, role])
                    .await?;
            }
        }
    ...
    pub async fn delete_user_role(user_id: impl ToString) -> Result<bool> {
        let _ = casbin_write!()
            .remove_filtered_grouping_policy(0, vec_string![user_id])
            .await?;
        Ok(true)
    }
}
