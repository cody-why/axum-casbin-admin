...
use crate::vo::sys_role_vo::RoleListData;
...
use super::sys_role_service::get_role_menu_ids;
...
pub async fn query_user_role(item: UserIdReq) -> Result<QueryUserRoleData> {
    let rb = pool!();
...
pub async fn query_user_menu(content: UserContext) -> Result<QueryUserMenuData> {
    ...
    let sys_menu_list: Vec<SysMenu> = if is_admin {
        ...
        // tracing::warn!("menu_ids: {:?}", menu_ids);
    ...
    for x in &sys_menu_list {
        // 菜单类型为3的为按钮
        if x.menu_type.is_some_and(|x| x != 3) {
            if let Some(id) = x.id {
                sys_menu_ids.insert(id);
            }
            if let Some(parent_id) = x.parent_id {
                if parent_id != 0 {
                    sys_menu_ids.insert(parent_id);
                }
            }
        }
...
pub async fn user_update(item: UserUpdateReq) -> Result<u64> {
    ...
    let rb = pool!();
    let result = SysUser::select_by_id_cache(rb, id).await?;

    match result.first() {
        None => Error::err("用户不存在"),
        Some(_user) => {
            let sys_user = SysUserUpdate::from(item);
            let result = SysUserUpdate::update_by_column(rb, &sys_user, "id").await?;
...
// 重置用户密码
pub async fn reset_user_password(item: UserIdReq) -> Result<u64> {
    let rb = pool!();
    let id = item.user_id;
    let password = "123456";
    let password = Password::md5_and_hash(password);
    let result = SysUser::update_password(rb, id, &password).await?;
    SysUser::remove_by_id_cache(id);
    if result.rows_affected == 1 {
        Ok(1)
